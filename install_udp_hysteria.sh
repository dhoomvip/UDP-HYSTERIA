...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - UDP-HYSTERIA server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install UDP-HYSTERIA$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified UDP-HYSTERIA binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove UDP-HYSTERIA$(treset)"
	echo -e "\t$0 --remove"
	echo
...
tpl_hysteria_server_service_base() {
  ...
  cat << EOF
[Unit]
Description=UDP-HYSTERIA Service
	...
	if ! is_hysteria_installed; then
		_is_frash_install=1
						...
						if [[ -n "$_is_frash_install" ]]; then
							echo
							echo -e "$(tbold)Congratulation! UDP-HYSTERIA has been successfully installed on your server.$(treset)"
							echo
							echo -e "$(tbold)Client app DHOOM VPN PRO:$(treset)"
							echo -e "$(tblue)https://play.google.com/store/apps/details?id=dvp.dhoomvpnpro.xyz$(treset)"
							echo
							echo -e "Follow me!"
							echo
							echo -e "\t+ Follow me on Telegram channel: $(tblue)https://t.me/mooa_322$(treset)"
							echo -e "\t+ Follow me on Telegram group: $(tblue)https://t.me/mooa_321$(treset)"
							echo -e "\t+ Message me on Telegram: $(tblue)https://t.me/mooa322$(treset)"
							echo
							else
								restart_running_services
								start_services
								echo
								echo -e "$(tbold)UDP-HYSTERIA has been successfully update to $VERSION.$(treset)"
								echo
								fi
}
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! UDP-HYSTERIA has been successfully removed from your server.$(treset)"
	echo
